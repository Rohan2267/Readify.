      ...
      {Data && (
        <div className='px-4 md:px-12 py-8 bg-zinc-900 flex flex-col lg:flex-row '>
          <div className='   w-full lg:w-3/6'>
            <div className='flex  flex-col lg:flex-row p-12 rounded justify-around bg-zinc-800'>
              <img src={Data.url} alt="/" className=' h-[50vh] md:h-[60vh] lg:h-[70vh] rounded' />
              {isLoggedIn === true && role === "user" && (<div className='flex flex-col md:flex-row  lg:flex-col items-center justify-between lg:justify-start lg:mt-0 mt-4 '>
                <button className='bg-white rounded  lg:rounded-full text-3xl p-3 text-red-500 flex items-center justify-center '
                  onClick={handleFavourite}><FaHeart />
                  <span className='ms-4 block lg:hidden  '>Favourites </span></button>
                <button className='text-white rounded lg:rounded-full text-3xl p-3 md:mt-0  mt-8 lg:mt-8 flex items-center justify-center bg-blue-500'
                onClick={handleCart}><FaShoppingCart />
                  <span className='ms-4 block lg:hidden  '>Add to Cart </span></button>

              </div>)}
              {isLoggedIn === true && role === "admin" && (<div className='flex flex-col md:flex-row  lg:flex-col items-center justify-between lg:justify-start lg:mt-0 mt-4 '>
                <Link
                  to={`/updateBook/${id}`}
